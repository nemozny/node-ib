...
Socket.prototype._onConnect = function () {
  ...
  if (C.USEV100PLUS) {
    ...
    const v100version = `v${C.MIN_SERVER_VER.FRACTIONAL_POSITIONS}..${C.MIN_SERVER_VER.PRICE_MGMT_ALGO}`

    this._client.write(v100prefix + _prefix(v100version) + ' ')
...
Socket.prototype.send = function (tokens, async) {
  ...
  if (C.USEV100PLUS) {
    this._client.write(_prefix(data));
  } else {
